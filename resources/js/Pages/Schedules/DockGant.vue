      ...
      <div id="gantt-date" class="h-16">
        ...
        <!-- //カレンダーの縦線 -->
          ...
          <div v-for="(bar, index) in taskBars" :key="index" class="flex flex-col h-36">
            ...
            :class="['rounded-lg absolute h-5 text-center text-xs has-tooltip',getBarColor(projectStyle.category_id)]" v-if="bar.projectStyles"  @dblclick="editOpen(projectStyle.id)">
              <div :id="'tooltip-'+projectStyle.s_id+'-'+index" class="tooltip"
